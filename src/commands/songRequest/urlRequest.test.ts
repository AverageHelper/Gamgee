...
describe("Song request via URL", () => {
  ...
  test("When there's a cooldown, only a user's first submission gets in", async () => {
    ...
    async function firstRequest(): Promise<void> {
      await urlRequest.execute(context1);
    ...
    async function secondRequest(): Promise<void> {
    ...
    // queue.push should only have been called once
