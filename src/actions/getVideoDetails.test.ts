...
describe("Video details", () => {
  ...
  `(
	...
	test.each`
	  desc                               | url
	  ${"long link of track"}            | ${"https://pony.fm/tracks/46025-beneath-the-sea-ft-lectro-dub-studio-quinn-liv-learn-zelizine"}
	  ${"incomplete long link of track"} | ${"https://pony.fm/tracks/46025-beneath-the-sea-ft-"}
	  ${"short link of track"}           | ${"https://pony.fm/t46025"}
	`(
		"returns correct length for $desc",
		async ({ url }: { desc: string, url: string }) => {
			const details = await getVideoDetails(url, null);
			expect(details).toHaveProperty("url", "https://pony.fm/tracks/46025-beneath-the-sea-ft-lectro-dub-studio-quinn-liv-learn-zelizine");
			expect(details?.duration.seconds).toBeDefined();
